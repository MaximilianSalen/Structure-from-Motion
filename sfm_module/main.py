...
def main():
    ...
    X0, desc_X, absolute_rotations = run_reconstruction(
        RT_list, init_imgs, K, init_pair[0]
    )

    # estimate T robustly and store
    initial_Ts = []
    for i in range(nr_images):
        init_T, X_corrs, x_corrs = get_T_and_correspondences(
            img_names[i], desc_X, X0, K, absolute_rotations[i], pixel_threshold
        )
