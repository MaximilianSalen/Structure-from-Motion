...
def test():
    ...
    data_root = "/home/simsom/work_space/repos/Structure-from-motion/"
    ...
    X0, absolute_rotations, inliers = run_reconstruction(R_list, initial_pair[0], initial_pair[1], K, init_pair[0], pixel_threshold)
    ...
    for i in range(nr_images):
        ...
        X_corr_inlier, x_corr_norm_inlier = get_correspondences(img_names[i], desc_X_inliers, X0, K)
    ...
    for i, init_T in enumerate(initial_Ts):
        refined_T = levenberg_marquardt_optimize_T(K, absolute_rotations[i], X_corr_inliers[i], x_corr_norm_inliers[i], init_T, num_iterations=10, mu=0.01)
    ...
    "purple", "brown", "pink", "gray", "olive", "navy",    
    "lightblue", "lightgreen", "coral", "beige", "indigo", 
    "lime", "maroon", "teal", "rose", "mustard", 
    "turquoise", "sienna", "plum", "orchid", "lavender"]
    for i_camera in range(nr_images-1):
        ...
        X_triangulated = auxiliary.triangulate_3D_point_DLT([P1, P2], [x1, x2])
        X_filtered = auxiliary.filter_3D_points(X_triangulated)
        auxiliary.plot_3d_points_and_cameras_new(X_filtered, [P1, P2], ax, colors[i_camera])
    plt.show()
